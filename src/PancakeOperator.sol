...
contract PancakeOperator is IPancakeOperator, IWithdrawFungibleCallback, IWithdrawFungiblesCallback {
    enum Function {
	None,
	AddLiquidity,
	...
	callbackResult = BOOKKEEPER.withdrawFungibles(params.positionId, tokens, amounts, address(this), callbackParams);
	...
	BOOKKEEPER.depositNonFungible(params.positionId, address(NPM), tokenId);
	...
	if (amount0 < params.amount0Desired) {
	    ...
	    BOOKKEEPER.depositFungible(params.positionId, params.token0);
	}
	...
	if (amount1 < params.amount1Desired) {
	    ...
	    BOOKKEEPER.depositFungible(params.positionId, params.token1);
	}
    ...
    {
	bytes memory result = BOOKKEEPER.withdrawFungible(
	...
	BOOKKEEPER.depositFungible(params.positionId, params.tokenOut);
